...
for act in "${@}"; do
    ...
    case "${act}" in
        ...
        sync)
            ...
            ;;
        changes)
            VERSION="$(sed -n "/__version__[ ]*=/s/.*['\"]\([0-9]\+\.[0-9]\+\.[0-9]\+.*\)['\"].*/\1/p" ../../clorm/__init__.py)"
            BUILD=$(curl -sL http://ppa.launchpad.net/potassco/${ref}/ubuntu/pool/main/p/python3-clorm/ | sed -n "/${VERSION//./\\.}-${rep}[0-9]\+\.dsc/s/.*${rep}\([0-9]\+\).*/\1/p" | sort -rn | head -1)
            cat > ${rep}/debian/changelog <<EOF
python3-clorm (${VERSION}-${rep}$[BUILD+1]) ${rep}; urgency=medium
...
EOF
        ...
        put)
            VERSION="$(head -n 1 ${rep}/debian/changelog | grep -o '[0-9]\+\.[0-9]\+\.[0-9]\+\(-[a-z0-9]\+\)\?')"
            dput ppa:potassco/${ref} python3-clorm_${VERSION}_source.changes
            ;;
        clean)
            rm -rf \
                "${rep}"/clorm \
                ...
                "${rep}"/debian/python3-clorm.debhelper.log \
                "${rep}"/debian/python3-clorm.substvars \
                "${rep}"/debian/python3-clorm \
