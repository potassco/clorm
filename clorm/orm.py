...
class FieldPath(object):
    '''Field Path provides a path from a predicate to an individual field. It
    specifies the full chain of links to the field through any complex terms.
    For example, a path 'Pred.a.b' for a predicate 'Pred' with a field 'a' an a
    subfield 'b'. Because a field can also be identified using positional
    arguments we can also specify a field path using positional arguments
    ...
    FieldPath is an internal library class and isn't directly exposed in the
    API. Instances are generated by FieldPathBuilders that are associated with
