...
class FactBase(object):
    ...
    def _init(self, facts=None, indexes=None):
        ...
        for path in self._indexes:
            if path.meta.predicate not in grouped:
                grouped[path.meta.predicate] = []
    ...
    def _check_init(self):
        if self._delayed_init:
            self._delayed_init()  # Check for delayed init
    ...
    # --------------------------------------------------------------------------
    def __init__(
        self, facts: Optional[_Facts] = None, indexes: Optional[Iterable[PredicatePath]] = None
    ) -> None:
    ...
    def select(self, root):
        ...
        for ptype in ptypes:
            self._factmaps.setdefault(ptype, FactMap(ptype))
    ...
    def asp_str(self, *, width: int = 0, commented: bool = False, sorted: bool = False) -> str:
        ...
        first = True
        if sorted:
            names = _builtin_sorted(
                self._factmaps.keys(), key=lambda pt: (pt.meta.name, pt.meta.arity, pt.__name__)
            )
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, self.__class__):
            other = FactBase(other)
        self._check_init()
        other._check_init()  # Check for delayed init

        self_fms = {p: fm for p, fm in self._factmaps.items() if fm}
        other_fms = {p: fm for p, fm in other._factmaps.items() if fm}
        if self_fms.keys() != other_fms.keys():
            return False

        for p, fm1 in self_fms.items():
            fm2 = other_fms[p]
            if not factset_equality(fm1.factset, fm2.factset):
                return False
    ...
    def __lt__(self, other):
        ...
        self_fms = {p: fm for p, fm in self._factmaps.items() if fm}
        other_fms = {p: fm for p, fm in other._factmaps.items() if fm}
        if len(self_fms) > len(other_fms):
            return False

        known_ne = False
        for p, spfm in self_fms.items():
            if p not in other_fms:
                return False
            opfm = other_fms[p]
            if spfm.factset < opfm.factset:
                known_ne = True
            elif spfm.factset > opfm.factset:
                return False
    ...
    def union(self, *others: _Facts) -> "FactBase":
        ...
        for o in factbases:
            predicates.update(o._factmaps.keys())
    ...
    def intersection(self, *others: _Facts) -> "FactBase":
        ...
        fb = FactBase()
        predicates = set(self._factmaps.keys())
        for fb_ in factbases:
            predicates.intersection_update(fb_._factmaps.keys())
    ...
    def symmetric_difference(self, other: _Facts) -> "FactBase":
        ...
        for p in predicates:
            in_self = p in self._factmaps
            in_other = p in other._factmaps
    ...
    def update(self, *others: _Facts) -> None:
        ...
        for fb in factbases:
            for p, fm in fb._factmaps.items():
                if p in self._factmaps:
                    self._factmaps[p].update(fm)
                else:
                    self._factmaps[p] = fm.copy()
    ...
    def intersection_update(self, *others: _Facts) -> None:
        ...
        predicates = set(self._factmaps.keys())
        for fb in factbases:
            predicates.intersection_update(fb._factmaps.keys())
        pred_to_delete = set(self._factmaps.keys()) - predicates

        for p in pred_to_delete:
            self._factmaps[p].clear()
        for p in predicates:
            pothers = [o._factmaps[p] for o in factbases if p in o._factmaps]
    ...
    def symmetric_difference_update(self, other: _Facts) -> None:
        ...
        for p in predicates:
            if p in self._factmaps and p in other._factmaps:
                ...
                if p in other._factmaps:
                    self._factmaps[p] = other._factmaps[p].copy()
