...
def ph_(value, *args, **kwargs):
    ...
    if len(args) > 1:
        raise TypeError(
            ("ph_() takes from 0 to 2 positional" "arguments but {} given").format(len(args) + 1)
        )

    # Set the default argument
    if "default" in kwargs:
        default = default = (True, kwargs["default"])
    elif len(args) > 0:
        default = (True, args[0])
    else:
        default = (False, None)
...
class MembershipSeq(object):
    ...
    def fixed(self):
        if isinstance(self._src, Placeholder):
            raise ValueError(("Cannot fix unground sequence specification : " "{}").format(self))
    ...
    def ground(self, *args, **kwargs):
        def get(arg):
            ...
            if isinstance(arg, PositionalPlaceholder):
                ...
                raise ValueError(
                    (
                        "Missing positional placeholder argument '{}' "
                ...
                raise ValueError(
                    (
                        "Missing named placeholder argument '{}' "
        ...
        elif not isinstance(self._src, Placeholder):
            return self
    ...
    def placeholders(self):
        ...
        elif not isinstance(self._src, Query):
            return set()
        where = self._src.qspec.where
        if where is None:
            return set()
...
def where_comparison_op(qcond):
    ...
    if all(map(_is_static_op_arg, newargs)):
        raise ValueError(
            (
                "Invalid comparison of only static inputs "
                "(at least one argument must reference a "
                "a component of a fact): {}"
            ).format(qcond)
        )
    ...
    if not spec.where and spec.join:
        raise ValueError(
            (
                "Invalid 'where' comparison operator '{}' is only "
...
def where_membership_op(qcond):
    ...
    if not isinstance(pth, PredicatePath):
        raise ValueError(
            ("Invalid 'where' condition '{}': missing path in " "membership declaration").format(
                qcond
            )
        )
    if isinstance(seq, PredicatePath):
        raise ValueError(
            (
                "Invalid 'where' condition '{}': invalid sequence in " "membership declaration"
            ).format(qcond)
        )
...
def join_comparison_op(qcond):
    ...
    if len(roots) != 2:
        raise ValueError(
            (
                "Invalid join expression '{}'. A join expression must join "
    ...
    if qcond.operator == trueall:
        if paths != roots:
            raise ValueError(
                ("Cross-product expression '{}' must contain only " "root paths").format(qcond)
            )
...
class StandardComparator(Comparator):
    ...
    def from_where_qcondition(cls, qcond):
        if not isinstance(qcond, QCondition):
            raise TypeError(
                (
                    "Internal bug: trying to make StandardComparator() "
                    "from non QCondition object: {}"
                ).format(qcond)
            )
        ...
        if not spec.where:
            raise ValueError(
                ("Invalid 'where' comparison operator " "'{}'").format(qcond.operator)
            )
    ...
    def from_join_qcondition(cls, qcond):
        if not isinstance(qcond, QCondition):
            raise TypeError(
                ("Internal bug: trying to make Join() " "from non QCondition object: {}").format(
                    qcond
                )
            )
        ...
        if not spec.join and spec.where:
            raise ValueError(
                (
                    "Invalid 'join' comparison operator '{}' is only "
                    "valid for a join specification"
                ).format(qcond.operator)
            )
        if not spec.join:
            raise ValueError(
                ("Invalid 'join' comparison operator " "'{}'").format(qcond.operator)
            )
    ...
    def fixed(self):
        ...
        if gself._operator not in [operator.contains, notcontains]:
            return gself
        elif isinstance(gself._args[0], frozenset):
            return gself
        elif not isinstance(gself._args[0], MembershipSeq):
            raise ValueError(
                ("Internal error: unexpected sequence type object: " "'{}'").format(
                    gself._args[0]
                )
            )
        return StandardComparator(self._operator, [gself._args[0].fixed(), gself._args[1]])
    ...
    def ground(self, *args, **kwargs):
        ...
        if self._operator not in [operator.contains, notcontains] or not isinstance(
            self._args[0], MembershipSeq
        ):
    ...
    def placeholders(self):
        ...
        if self._operator not in [operator.contains, notcontains]:
            return tmp
        elif not isinstance(self._args[0], MembershipSeq):
            return tmp
    ...
    def preference(self):
        ...
        if pref is None:
            raise ValueError(
                ("Operator '{}' does not have a join " "preference").format(self._operator)
            )
        return pref
    ...
    def make_callable(self, root_signature):
        for arg in self._args:
            if isinstance(arg, Placeholder):
                raise TypeError(
                    ("Internal bug: cannot make a non-ground " "comparator callable: {}").format(
                        self
                    )
                )
...
class FunctionComparator(Comparator):
    def __init__(self, func, path_signature, negative=False, assignment=None):
        ...
        if len(funcsig.parameters) < len(self._pathsig):
            raise ValueError(
                (
                    "More paths specified in the path signature '{}' "
                    "than there are in the function "
                    "signature '{}'"
                ).format(self._pathsig, funcsig)
            )
        ...
        for pp in self._pathsig:
            if not isinstance(pp, PredicatePath.Hashable):
                raise TypeError(
                    ("The boolean functor call signature must " "consist of predicate paths")
                )
    ...
    def _check_assignment(self):
        ...
        if tmp:
            raise ValueError(
                (
                    "FunctionComparator is being given "
    ...
    def executable(self):
        for ph in self._placeholders:
            if not ph.has_default:
                return False
        return True

    def negate(self):
        neg = not self._negative
        return FunctionComparator(self._func, self._pathsig, neg, assignment=self._assignment)
    ...
    def dealias(self):
        ...
        return FunctionComparator(
            self._func, newpathsig, self._negative, assignment=self._assignment
        )
    ...
    def ground(self, *args, **kwargs):
        ...
        for k, v in kwargs.items():
            if k in assignment:
                raise ValueError(
                    ("Both positional and keyword values given " "for the argument '{}'").format(
                        k
                    )
                )
            assignment[k] = v
        ...
        return FunctionComparator(self._func, self._pathsig, self._negative, assignment)

    def make_callable(self, root_signature):
        if self._assignment is None:
            raise RuntimeError(
                ("Internal bug: make_callable called on a " "ungrounded object: {}").format(self)
            )
...
def make_input_alignment_functor(input_root_signature, output_signature):
    ...
    def validate_input_signature():
        ...
        try:
            ...
            raise TypeError(
                ("Invalid input predicate path signature {}: " "{}").format(
                    input_root_signature, e
                )
            ) from None
        return inputs
    ...
    def validate_output_signature():
        ...
        for a in output_signature:
            ...
            if isinstance(a, Placeholder):
                raise TypeError(
                    ("Output signature '{}' contains a placeholder " "'{}'").format(
                        output_signature, a
                    )
                )
        return outputs
    ...
    for out in outsig:
        if isinstance(out, PredicatePath):
            ...
            if idx is None:
                raise TypeError(
                    (
                        "Invalid signature match between {} and {}: "
                        "missing input predicate path for "
                        "{}"
                    ).format(input_root_signature, output_signature, out)
                )
    ...
    def func(facts):
        try:
            ...
            raise TypeError(
                (
                    "Invalid input to getter function: expecting "
                    "a tuple with {} elements and got a tuple with "
                    "{}"
                ).format(len(insig), len(facts))
            ) from None
        except TypeError as e:
            raise TypeError(("Invalid input to getter function: " "{}").format(e)) from None
...
g_bool_operators = {operator.and_: True, operator.or_: True, operator.not_: True}
...
def is_boolean_qcondition(cond):
    ...
    if isinstance(cond, FuncInputSpec):
        return False
    elif not isinstance(cond, QCondition):
        return False
...
def validate_where_expression(qcond, roots=[]):
    ...
    try:
        ...
        raise ValueError(
            ("Invalid predicate paths signature {}: " "{}").format(roots, e)
        ) from None
    for pp in roots:
        if not pp.path.meta.is_root:
            raise ValueError(
                (
                    "Invalid roots element {} does not refer to " "the root of a predicate path "
                ).format(pp)
            )
    ...
    def check_path(path):
        if hashable_path(path.meta.root) not in roots:
            raise ValueError(
                (
                    "Invalid 'where' expression '{}' contains a path "
                    "'{}' that is not a sub-path of one of the "
                    "roots '{}'"
                ).format(qcond, path, roots)
            )
    ...
    def is_static_condition(cond):
        ...
        if callable(cond):
            raise TYpeError(
                ("Internal bug: invalid static test " "with callable: {}").format(cond)
            )
        return True
    ...
    def validate_callable(func):
        if len(roots) != 1:
            raise ValueError(
                (
                    "Incompatible usage between raw functor {} and " "non-singleton predicates {}"
                ).format(func, roots)
            )
    ...
    def validate_condition(cond):
        if isinstance(cond, Placeholder):
            raise ValueError(
                (
                    "Invalid 'where' condition '{}' in query '{}': a "
        ...
        elif isinstance(cond, Comparator):
            return cond
        elif is_boolean_qcondition(cond):
            return validate_bool_condition(cond)
        elif is_comparison_qcondition(cond):
...
def negate_where_expression(qcond):
    ...
    def negate_bool_condition(bcond):
        ...
        if bcond.operator == operator.and_:
            return or_(negate_condition(bcond.args[0]), negate_condition(bcond.args[1]))
        if bcond.operator == operator.or_:
            return and_(negate_condition(bcond.args[0]), negate_condition(bcond.args[1]))
        raise TypeError(
            ("Internal bug: unknown boolean operator '{}' in query " "condition '{}'").format(
                bcond.operator, qcond
            )
        )
    ...
    def negate_condition(cond):
        ...
        if not is_boolean_qcondition(cond):
            raise TypeError(
                ("Internal bug: unexpected non-boolean condition '{}' " "in query '{}'").format(
                    cond, qcond
                )
            )
...
def where_expression_to_cnf(qcond):
    ...
    def condition_to_cnf(cond):
        ...
        if not is_boolean_qcondition(cond):
            raise TypeError(
                (
                    "Internal bug: unexpected non-boolean condition '{}' "
                    "in 'where' expression '{}'"
                ).format(cond, qcond)
            )
...
class Clause(object):
    def __init__(self, comparators):
        ...
        for comp in comparators:
            if not isinstance(comp, Comparator):
                raise ValueError(
                    ("Internal bug: only Comparator objects " "allowed: {} ").format(comp)
                )
    ...
    @property
    def placeholders(self):
        return set(itertools.chain.from_iterable([p.placeholders for p in self._comparators]))
...
class ClauseBlock(object):
    def __init__(self, clauses=[]):
        ...
        for clause in self._clauses:
            if not isinstance(clause, Clause):
                raise ValueError(
                    (
                        "A ClauseBlock must consist of a list of "
    ...
    @property
    def placeholders(self):
        return set(itertools.chain.from_iterable([c.placeholders for c in self._clauses]))
    ...
    def stack_add(cond):
        if is_leaf(cond):
            ...
            for arg in cond.args:
                if cond.operator == operator.and_:
                    stack.append(NEWCL)
                stack_add(arg)
                if cond.operator == operator.and_:
                    stack.append(NEWCL)
...
def partition_clauses(clauses=[]):
    ...
    if catchall:
        return (clauseblocks, ClauseBlock(catchall))
    else:
        return (clauseblocks, None)
...
def validate_join_expression(qconds, roots):
    ...
    def add_join(join):
        ...
        if len(jr) != 2:
            raise ValueError(
                (
                    "Internal bug: join specification should have " "exactly two root paths: '{}'"
                ).format(jr)
            )
        x, y = jr
        ...
        if remain:
            raise ValueError(
                ("Join specification '{}' contains unmatched " "root paths '{}'").format(
                    jr, remain
                )
            )
    ...
    for p in broots:
        if not p.path.meta.is_root:
            raise ValueError(
                (
                    "Invalid field specification {} does not refer to "
    ...
    for qcond in _process_join_with_andop(qconds):
        if not is_join_qcondition(qcond):
            if not isinstance(qcond, QCondition):
                raise ValueError(
                    (
                        "Invalid join element '{}': expecting a "
                ...
                raise ValueError(
                    ("Invalid join operator '{}' in " "{}").format(qcond.operator, qcond)
                )
    ...
    if jroots != broots:
        raise ValueError(
            ("Invalid join specification: missing joins for " "'{}'").format(broots - jroots)
        )
    if not is_connected():
        raise ValueError(
            ("Invalid join specification: contains un-joined " "components '{}'").format(qconds)
        )
    ...
    def _prevalidate():
        for j in join_expression:
            if not isinstance(j, QCondition):
                raise TypeError(
                    "'{}' ({}) is not a valid 'join' in '{}'".format(j, type(j), join_expression)
                )
...
class OrderBy(object):
    ...
    def __str__(self):
        if self._asc:
            return "asc({})".format(self._path)
        else:
            return "desc({})".format(self._path)
...
def validate_orderby_expression(orderby_expressions, roots=[]):
    ...
    for exp in orderby_expressions:
        if isinstance(exp, OrderBy):
            ...
            raise ValueError("Invalid 'order_by' expression: {}".format(exp))
    obb = OrderByBlock(path_ordering)

    if not obb.roots.issubset(hroots):
        raise ValueError(
            (
                "Invalid 'order_by' expression '{}' refers to root paths that " "are not in '{}'"
            ).format(obb, hroots)
        )
    return obb
...
def make_prejoin_pair(indexed_paths, clauseblock):
    ...
    def is_candidate(indexes, cl):
        for c in cl:
            if not isinstance(c, StandardComparator):
                return False
            if not is_candidate_sc(indexes, c):
                return False
        return True
    ...
    indexes = set(filter(lambda x: x in tmp, [hashable_path(p) for p in indexed_paths]))
...
def make_join_pair(joins, clauseblock, orderbys=[]):
    ...
    if remainder:
        ...
        if clauseblock:
            return (joinsc, clauseblock + remainder)
        else:
            return (joinsc, remainder)
    else:
        if clauseblock:
            return (joinsc, clauseblock)
        else:
            return (joinsc, None)
...
def _align_sc_path(root, sc):
    ...
    if not sc:
        return None
    if isinstance(sc.args[0], PredicatePath) and hashable_path(sc.args[0].meta.root) == hroot:
        return sc
    sc = sc.swap()
    if not isinstance(sc.args[0], PredicatePath) or hashable_path(sc.args[0].meta.root) != hroot:
        raise ValueError(
            (
                "Cannot align key comparator '{}' with root '{}' since "
...
class JoinQueryPlan(object):
    ...
    def __init__(
        ...
        self._indexes = tuple([p for p in indexes if path(p).meta.predicate == self._predicate])
        ...
        if self._joinsc and self._joinsc.placeholders:
            raise ValueError(
                (
                    "A comparator with a placeholder is not valid as " "a join specificaton: {}"
                ).format(joinsc)
            )
        ...
        if not _check_roots(allroots, joinsc):
            raise ValueError(
                ("Join comparator '{}' refers to non '{}' " "paths").format(joinsc, allroots)
            )
        if not _check_roots(allroots, postjoincb):
            raise ValueError(
                ("Post-join clause block '{}' refers to non '{}' " "paths").format(
                    postjoincb, allroots
                )
            )
        ...
        if not _check_roots(allroots, postjoinobb):
            raise ValueError(
                ("Post-join order by block '{}' refers to non '{}' " "paths").format(
                    postjoinobb, allroots
                )
            )
    ...
    def from_specification(
        cls, indexes, input_signature, root, joins=[], clauses=[], orderbys=[]
    ):
        def _paths(inputs):
        ...
        if not rootcbs:
            ...
            raise ValueError(
                (
                    "Internal bug: unexpected multiple single root "
    ...
    def ground(self, *args, **kwargs):
        ...
        if (
            gprejoincl == self._prejoincl
            and gprejoincb == self._prejoincb
            and gpostjoincb == self._postjoincb
        ):
            return self
    ...
    def __eq__(self, other):
        if not isinstance(other, self.__class__):
            return NotImplemented
        if self._insig != other._insig:
            return False
        if self._root.meta.hashable != other._root.meta.hashable:
            return False
        if self._prejoincl != other._prejoincl:
            return False
        if self._prejoincb != other._prejoincb:
            return False
        if self._prejoinobb != other._prejoinobb:
            return False
        if self._joinsc != other._joinsc:
            return False
        if self._postjoincb != other._postjoincb:
            return False
        return True
...
class QueryPlan(object):
    def __init__(self, subplans):
        ...
        for jqp in subplans:
            ...
            if sig != insig:
                raise ValueError(
                    (
                        "Invalid 'input_signature' for JoinQueryPlan. "
                        "Got '{}' but expecting '{}'"
                    ).format(insig, sig)
                )
    ...
    @property
    def placeholders(self):
        return set(itertools.chain.from_iterable([jqp.placeholders for jqp in self._jqps]))
...
def make_orderby_partitions(root_join_order, orderbys=[]):
    if not orderbys:
        return [OrderByBlock([]) for r in root_join_order]
...
def remove_orderby_gaps(partitions):
    def gap(partitions):
        ...
        for idx, obs in enumerate(partitions):
            if obs and startgap == -1:
                startgap = idx
            elif obs and startgap != -1 and startgap == idx - 1:
                startgap = idx
            elif obs and startgap != -1:
                return (startgap, idx)
        return (-1, -1)
...
class QuerySpec(object):
    ...
    def __init__(self, **kwargs):
        for k, v in kwargs.items():
            ...
            if v is None:
                raise ValueError(
                    (
                        "Error for QuerySpec parameter '{}': 'None' " "values are not allowed"
                    ).format(k)
                )
    ...
    def bindp(self, *args, **kwargs):
        ...
        for idx, v in enumerate(args):
            if idx not in pp:
                raise ValueError(
                    (
                        "Trying to bind value '{}' to positional "
    ...
    def __getattr__(self, item):
        if item not in QuerySpec.allowed:
            raise ValueError(
                ("Trying to get the value of unknown parameter " "'{}'").format(item)
            )
...
def fix_query_spec(inspec):
    ...
    return QuerySpec(roots=inspec.roots, join=join, where=where, order_by=order_by)
...
def make_query_plan_preordered_roots(indexed_paths, root_join_order, qspec):
    ...
    for idx, (root, rorderbys) in enumerate(zip(root_join_order, orderbygroups)):
        ...
        if rpclauses:
            rpclauses = ClauseBlock(rpclauses)
        ...
        output.append(
            JoinQueryPlan.from_specification(
                indexed_paths, root_join_order[:idx], root, rpjoins, rpclauses, rorderbys
            )
        )
...
def fixed_join_order(*roots):
    def validate(r):
        ...
        if not r.meta.is_root:
            raise ValueError(
                ("Bad query roots specification '{}': '{}' is not " "a root path").format(
                    roots, r
                )
            )
        return r
    ...
    if not roots:
        raise ValueError(
            "Missing query roots specification: cannot create "
    ...
    def fixed_join_order_heuristic(indexed_paths, qspec):
        ...
        if hps != set(hashables):
            raise ValueError(
                (
                    "Mis-matched query roots: fixed join order "
                    "heuristic '{}' must contain exactly the "
                    "roots '{}"
                ).format(roots, qspec.roots)
            )
        return list(paths)
...
def oppref_join_order(indexed_paths, qspec):
    ...
    return [path(hrp) for hrp in sorted(root2val.keys(), key=lambda k: root2val[k], reverse=True)]
...
class InQuerySorter(object):
    def __init__(self, orderbyblock, insig=None):
        if insig is None and len(orderbyblock.roots) > 1:
            raise ValueError(
                (
                    "Cannot create an InQuerySorter with no input "
    ...
    if pjk:
        ...
        for sc in pjk:
            ...
            if keyable is None:
                raise ValueError(
                    (
                        "Internal error: prejoin key clause '{}' "
...
def make_first_join_query(jqp, factsets, factindexes):
    ...
    if jqp.input_signature:
        raise ValueError(
            ("A first JoinQueryPlan must have an empty input " "signature but '{}' found").format(
                jqp.input_signature
            )
        )
    if jqp.prejoin_orderbys and jqp.postjoin_orderbys:
        raise ValueError(
            (
                "Internal error: it doesn't make sense to have both "
...
def make_prejoin_query_source(jqp, factsets, factindexes):
    ...
    if pjc:
        ...
        if len(pjc.roots) != 1 and pjc_root.meta.predicate != predicate:
            raise ValueError(
                (
                    "Internal error: prejoin clauses '{}' is invalid " "for JoinQueryPlan {}"
                ).format(pjc, jqp)
            )
    ...
    if jk:
        ...
        if jk.args[0].meta.predicate != predicate:
            raise ValueError(
                ("Internal error: join key '{}' is invalid " "for JoinQueryPlan {}").format(
                    jk, jqp
                )
            )

    if pjob:
        pjiqs = InQuerySorter(pjob)
    else:
        pjiqs = None
    ...
    def query_source():
        if jk:
            ...
            if not pjc and not pjk and not pjob:
                ...
                source = [f for (f,) in query_pjc()]
            elif pjk:
                source = [f for (f,) in query_pjk()]
...
def make_chained_join_query(jqp, inquery, factsets, factindexes):
    ...
    if not jqp.input_signature:
        raise ValueError(
            (
                "A non-first JoinQueryPlan must have a non-empty input "
                "signature but '{}' found"
            ).format(jqp.input_signature)
        )
    ...
    def query_jk():
        ...
        align_query_input = make_input_alignment_functor(jqp.input_signature, (jk.args[1],))
        fi = query_source()
    ...
    if jk:
        unsorted_query = query_jk
    else:
        unsorted_query = query_no_jk
    if not postj_order:
        return unsorted_query
...
def make_query(qp, factsets, factindexes):
    if qp.placeholders:
        raise ValueError(
            (
                "Cannot execute an ungrounded query. Missing values " "for placeholders: " "{}"
            ).format(", ".join([str(p) for p in qp.placeholders]))
        )
    query = None
    for idx, jqp in enumerate(qp):
        if not query:
            query = make_first_join_query(jqp, factsets, factindexes)
        else:
...
def make_outputter(queryroots, insig, outsig):
    ...
    if needcomplex:
        return make_complex_outputter()
    else:
        return make_simple_outputter()
...
class QueryExecutor(object):
    ...
    def _make_plan_and_query(self):
        ...
        if where and not where.executable:
            ...
            raise ValueError(
                ("Placeholders {} must be bound to values before " "executing the query").format(
                    phstr
                )
            )
        qspec = self._qspec
    ...
    def _group_by_all(self):
        ...
        if qspec.order_by:
            iqs = InQuerySorter(OrderByBlock(qspec.order_by), self._qplan.output_signature)
    ...
    def delete(self):
        ...
        if not subroots.issubset(set(roots)):
            raise ValueError(
                (
                    "For a 'delete' query the selected items '{}' "
        ...
        if (
            len(roots) == 1
            and len(subroots) == 1
            and not self._qspec.where
            and not self._qspec.join
        ):
    ...
    def _general_modify(self, name, fn):
        ...
        if not subroots.issubset(set(roots)):
            raise ValueError(
                (
                    f"For a '{name}' query the selected items '{selection}' "
