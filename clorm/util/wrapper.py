...
def _check_wrapper_object(wrapper, strict=False):
    ...
    if issubclass(ActualType, WrappedType):
        return
    if strict:
        raise TypeError(
            ("Invalid proxied object {} not of expected type " "{}").format(
...
def init_wrapper(wrapper: Any, *args: Any, **kwargs: Any) -> None:
    ...
    if "wrapped_" in kwargs:
        if len(args) != 0 and len(kwargs) != 1:
            raise ValueError(
                (
                    "Invalid initialisation: the 'wrapped_' argument "
...
class WrapperMetaClass(type):
    def __new__(meta, name, bases, dct):
        ...
        ignore = ["__init__", "__new__", "__dict__", "__weakref__", "__setattr__", "__getattr__"]

        if "_wrapped_cls" in dct:
            raise TypeError(
                (
                    "ProxyMetaClass cannot proxy a class with a " '"_wrapped_cls" attribute: {}'
                ).format(PrClass)
            )
...
def make_class_wrapper(inputclass: type, override: Optional[type] = None) -> type:
    ...
    if override:
        for key, value in override.__dict__.items():
            if key in o_ignore:
                continue
            if key == "__doc__" and not value:
                continue
            dct[key] = value

    if "_wrapped_cls" in dct:
        raise TypeError(
            ("The overrides cannot contain a " '"_wrapped_cls" attribute: {}').format(dct)
        )
    dct["_wrapped_cls"] = Wrapped
