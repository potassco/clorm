...
from clorm import Predicate, ComplexTerm, RawField, IntegerField, FactBase
...
def compare_generating_simple_facts_and_symbols():
    ...
    print("=============================================================")
    print(("Comparing the generation of simple facts (named and positional args) "
    ...
    print("Instantating {} simple Clingo symbols in {}".format(
        len(ssymbols), ssymbols_t))

    # Time to generate P facts
    (snfacts, snfacts_t) = generate_list(create_simple_fact_named)
    print("Instantating {} simple named Clorm facts in {}".format(
        len(snfacts), snfacts_t))

    (spfacts, spfacts_t) = generate_list(create_simple_fact_positional)
    print("Instantating {} simple positional Clorm facts in {}".format(
        len(spfacts), spfacts_t))

    # Time to generate basic python object
    (nobjects, nobjects_t) = generate_list(create_pts_named)
    print("Instantating {} basic named python objects in {}".format(
        len(nobjects), nobjects_t))

    # Time to generate basic python object
    (pobjects, pobjects_t) = generate_list(create_pts_positional)
    print("Instantating {} basic positional python objects in {}".format(
        len(pobjects), pobjects_t))
...
def compare_generating_complex_facts_and_symbols():
    ...
    print("Instantating complex {} Clorm facts in {}".format(
        len(cfacts), cfacts_t))

    # Time to generate clingo symbols
    (csymbols, csymbols_t) = generate_list(create_complex_symbol)
    print("Instantating complex {} Clingo symbols in {}".format(
        len(csymbols), csymbols_t))
...
def time_to_instantiate_simple_from_raw():
    ...
    with Timer() as unify_timer: foall = [PT._unify(s) for s in symbols]
...
def time_to_instantiate_complex_from_raw():
    ...
    with Timer() as unify_timer: foall = [P._unify(s) for s in symbols]
